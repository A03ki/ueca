...
class TestConstants:
    def test_c(self):
        assert c.magnitude == 2.99792458E8
    ...
    def test_epsilon_0(self):
        assert epsilon_0.magnitude == 8.8541878128E-12
        assert epsilon_0.unit == "farad / meter"

    def test_g(self):
        assert g.magnitude == 9.80665
        assert g.unit == "meter / second ** 2"

    def test_G(self):
        assert G.magnitude == 6.6743E-11
        assert G.unit == "meter ** 2 * newton / kilogram ** 2"

    def test_h(self):
        assert h.magnitude == 6.62607015E-34
        assert h.unit == "joule * second"

    def test_k(self):
        assert k.magnitude == 1.380649E-23
        assert k.unit == "joule / kelvin"

    def test_m_e(self):
        assert m_e.magnitude == 9.1093837015E-31
        assert m_e.unit == "kilogram"

    def test_m_p(self):
        assert m_p.magnitude == 1.67262192369E-27
        assert m_p.unit == "kilogram"

    def test_m_u(self):
        assert m_u.magnitude == 1.6605390666E-27
        assert m_u.unit == "kilogram"

    def test_mu_0(self):
        assert mu_0.magnitude == 1.25663706212E-6
        assert mu_0.unit == "henry / meter"

    def test_N_A(self):
        assert N_A.magnitude == 6.02214076E23
        assert N_A.unit == "1 / mole"

    def test_R(self):
        assert R.magnitude == 8.314462618
